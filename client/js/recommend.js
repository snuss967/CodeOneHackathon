...
var getCompoundInterestAndSetMessage = function(numberOfYears, annualAddition) {
	...
	{
		$('.advice-content').html("Bonds are debt issued by Countries, Cities, or Corporations in an attempt to raise money. They have maturity lengths of different terms but are a relatively safe way to keep your money and earn interest. However, they are not liquid.");
		$('.plan').html("An Assortment Of Bonds");
		return 0.06;
	}
	else if(numberOfYears <= 2 && annualAddition <= 9500)
	{
		$('.advice-content').html("You are going to need your money extremely soon and you don’t want to take any risk of losing it. A savings account is your best option.");
		$('.plan').html("A Savings Account");
		return 0.0008;
	}
	else if((numberOfYears <= 10 && numberOfYears >= 5) && annualAddition >= 7000)
	{
		$('.advice-content').html("A mutual fund is a collection of stocks that are managed and owned by a collection of people. Low risk mutual funds are funds that will generate returns that are usually lower than a high or moderate risk mutual fund, but the chance that one will lose a significant amount of money in the account is much lower. If you are willing to take more risk to see higher returns on your investment take a look at moderate risk mutual funds.");
		$('.plan').html("A Mutual Fund");
		return 0.08;
	}
	else if((numberOfYears <= 8 && numberOfYears >= 5) && annualAddition >= 3000)
	{
		$('.advice-content').html("You have a relatively moderate duration of time to invest and watch your money grow. You will most likely see the greatest returns during this time period by investing in index funds with your money.");
		$('.plan').html("An Index Fund");
		return 0.084;
	}
	else if(((numberOfYears <= 15 && numberOfYears >= 10)) && annualAddition <= 4000)
	{
		$('.advice-content').html("A mutual fund is a collection of stocks that are managed and owned by a collection of people. Moderate risk mutual funds are funds that will generate return rates that are usually higher than a low risk mutual fund, but lower than a high risk mutual fund. They run the risk of losing money but not as great as a High Risk mutual fund. Combined with some individidual stocks it allows you to take slightly more risk for the chance of having higher returns.");
		$('.plan').html("A Mutual Fund");
		return 0.092;
	}
	else if((numberOfYears <= 15 && numberOfYears >= 10) && annualAddition >= 4000)
